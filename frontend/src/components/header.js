...
const Header = () => {
    ...
    return (<nav className="navbar navbar-expand-lg navbar-dark bg-dark">
        <Link className="navbar-brand" to="/">
            <svg width="1em" height="1em" viewBox="0 0 16 16" className="bi bi-shield-check" fill="#45de4f" xmlns="http://www.w3.org/2000/svg" style={{ marginTop: '-4px' }}>
        ...
        <div className="collapse navbar-collapse " id="navbarSupportedContent">
            <ul className="navbar-nav ml-auto ">
                ...
                {localStorage.getItem('user') ? <li className="nav-item">
                    <span className="navbar-text" to="#" ><b className='text-info'>{AuthService.getCurrentUser().username}</b> [{AuthService.getCurrentUser().role}]</span>
                </li> : null}
                {localStorage.getItem('user') ? <li className="nav-item">
                    <Link className="nav-link text-danger" to="#" onClick={() => AuthService.logout()}> Logout </Link>
                </li> : null}
            </ul>
        </div>
    </nav>)
